...
1. **Dependency on Sprint 0.4 (`cold_start.sh`):** `bp_run` calls `cs_read_commit` and `cs_store_commit` from `cold_start.sh`. If Sprint 0.4 has not been implemented when this sprint starts, the test file must stub those functions. Should the spec formally allow the coding agent to provide stubs in the test file only (not in `boot_pull.sh` itself), or should the agent be required to block until Sprint 0.4 is merged? Recommendation: stubs in the test file are acceptable for CI isolation; `boot_pull.sh` itself must always call the real `cs_*` functions without fallback stubs. Orchestrator confirmation preferred before implementation begins.

2. **Sentinel touch on network error:** The current spec says do NOT touch the sentinel when `se_pull` returns 2 (network error). This means the runtime poll (Sprint 0.6) will use the old sentinel mtime as its baseline. Is that the correct behavior? The old baseline is still valid — we just didn't pull. Alternatively, touching the sentinel on network error would give the runtime poll a fresh baseline but means a save changed by another device between the last boot and this boot won't be reflected until connectivity is restored and a manual or runtime pull is triggered. The spec as written (no touch on network error) is conservative and safe. Confirm this is the desired behavior.

3. **`bp_apply_remote_saves` partial failure behavior:** The current spec returns 1 after processing all files if any file failed (criterion 9 — continues on `pm_repo_to_local` failure). For `cp` failures (criterion 10), the current spec returns 1 immediately. Should `cp` failures also continue processing remaining files? Immediate abort is safer (avoids partial state), but continuing gives the user more saves restored at the cost of a known incomplete sync. Recommendation: keep immediate abort on `cp` failure for now; revisit in Sprint 0.8 when conflict handling is in place. Confirm preference.
