...
1. **`se_stage_files` and untracked files.** Sprint 0.3 defined `se_stage_files` to stage files via `git add`. For conflict artifacts (`.local` and `.conflict`), these are new, untracked files — `git add -u` would not pick them up. Verify that the Sprint 0.3 `se_stage_files` implementation uses `git add` (not `git add -u`) and accepts a list of specific file paths. If it uses `git add -u`, either update `se_stage_files` or call `git add` directly in `ch_handle_pull_conflict` for the artifact files. Orchestrator should confirm before implementation begins.

2. **Branch name.** This spec assumes `main` as the primary branch. Sprint 0.3 enrollment spec should be checked to confirm the default branch. If enrollment uses a configurable branch name stored in config, `conflict_handler.sh` must read it from the same source rather than hardcoding `main`.

3. **Multiple `.local` files for the same save (multi-device conflicts).** If Device A and Device B both conflict with a remote save, the repo may accumulate `save.srm.device-a.local` and `save.srm.device-b.local`. `ch_resolve` uses `find` to locate the `.local` file and picks the first match via `head -1`. Is this the correct behavior for multi-device conflicts? The alternative is for `ch_resolve` to require the caller to specify the exact `.local` filename. For Sprint 0.8, `head -1` (using the first match) is acceptable — multi-device conflict UI (Sprint 1.2) can present a more nuanced choice. Confirm this is acceptable.

4. **`reset --hard` and uncommitted device saves.** The `reset --hard origin/main` in `ch_handle_pull_conflict` step 4 resets the working tree. If there are uncommitted changes to non-`.srm` files in the repo (e.g., a partially written `.continuity/devices/` update), those would be lost. Enumerate whether any other core module leaves uncommitted non-`.srm` changes in the repo working tree. If not, `reset --hard` is safe. Confirm before implementation.

5. **Cold start `.local` files and this sprint's convention.** Sprint 0.4 cold start creates `.local` files as `<repo_path>.<device_name>.local` (inferred from the roadmap, but the exact Sprint 0.4 spec uses `.<device_name>.local` without the explicit `.srm` intermediate). Verify that Sprint 0.4's `.local` naming is consistent with this sprint's convention (`snes/super_metroid.srm.my-brick.local`). If Sprint 0.4 used a different naming scheme (e.g. `snes/super_metroid.local`), `ch_list_local_files`'s parsing logic must accommodate both or Sprint 0.4's output must be standardized. Orchestrator to confirm naming consistency across sprints.
