...
1. **Should `sb_run` return 2 when offline and pull was skipped?** The current spec has `sb_run` return 0 on success even when the pull was skipped due to offline. The rationale: recovery still ran meaningfully (catch-up scan completed, local commits queued), and the device is in a valid state for runtime polling. Returning 2 would signal "network error" to the dispatcher, which might cause it to skip the catch-up scan. Recommendation: return 0 when offline during pull, since the outbound work still completes. Confirm this is the desired dispatcher contract.

2. **`sb_is_stale` when sentinel is absent and clean shutdown marker is present.** This can occur if `sb_mark_clean_shutdown` was called during enrollment or some other initialization path before the first cold start created the sentinel. The spec returns 1 (not stale) in this case, meaning `cs_is_cold_start` will catch it first in the dispatcher. The marker is harmless and will be consumed by `sb_run` when stale boot eventually runs after cold start. Confirm this edge case handling is correct.

3. **Should the `clean_shutdown` marker be named in `.gitignore` explicitly, or is `.srm`-only filtering in `cd_detect_changes` sufficient protection?** The `clean_shutdown` file does not end in `.srm`, so `cd_detect_changes` will never stage it. However, best practice and explicit safety suggest enrollment (Sprint 0.3) should add `clean_shutdown` to the `.continuity/.gitignore`. This sprint does not add it, relying on Sprint 0.3's existing `.gitignore` entry for all `.continuity/` local state files. If Sprint 0.3 wrote a blanket `.continuity/` gitignore rule, no action needed. Confirm with Sprint 0.3 implementor.

4. **Test 1 assertion 18 — `sb_is_stale` after a successful `sb_run`.** After `sb_run` completes, the clean shutdown marker remains absent (only the daemon's SIGTERM handler writes it). So `sb_is_stale` returns 0 on the next boot. This means the device always enters stale boot recovery unless the daemon successfully handled SIGTERM. This is the correct and safe behavior — each recovery run is idempotent. However, if the integration test framework calls `sb_run` a second time immediately after the first, it should detect no changes and exit cleanly. Confirm the idempotency test is sufficient validation.

5. **Catch-up scan and repo-only files.** The catch-up scan only iterates device saves (outbound only). Files that exist in the repo but NOT on the device are not removed from the repo — they are left as-is. Is this the desired behavior? Rationale: if a repo-only save exists, it was pushed by another device and should remain in the repo. It will be applied to the device on the next boot pull. For stale boot, deleting repo-only files would be destructive. Recommendation: leave repo-only files untouched. Confirm.
