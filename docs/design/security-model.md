...
| **Malicious OTA** | OTA fetches the PUBLIC project repo anonymously over TLS. `checksums.txt` and the CRLF scan verify INTEGRITY (torn/corrupt copies), not authenticity — they live in the same repo they describe. Authenticity reduces entirely to push access on the channel branch: protect the GitHub account (2FA) and the branch. `CONTINUITY_OTA=0` disables the mechanism. Signed manifests are a known possible upgrade if the project ever has multiple maintainers. |
